...
struct ShowDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                // Show Poster
                if let posterPath = discoverShow?.posterPath {
                ...
                HStack {
                    Text("Status: \(show.status.capitalized)")
                        .font(.headline)
                        .foregroundColor(show.ended ? .red : .green)
                    Spacer()
                    if show.monitored {
                        Text("Monitored")
                            .font(.subheadline)
                ...
                // Overview
                if let overview = discoverShow?.overview ?? show.overview {
                    Text("Overview")
                        .font(.title2)
                        .fontWeight(.semibold)
                    Text(overview)
                ...
                if let discoverShow = discoverShow {
                    Button("Add to Sonarr") {
                        addShowToSonarr(discoverShow)
                    }
    ...
    private func addShowToSonarr(_ show: DiscoverShow) {
        ...
        SonarrPlusViewModel.shared.fetchQualityProfiles { profiles in
            ...
            let qualityPicker = QualityPicker(profiles: profiles) { selectedProfile, startDownload in
                ...
                SonarrPlusViewModel.shared.addShow(show: show, qualityProfileId: selectedProfile.id, startDownload: startDownload) {
